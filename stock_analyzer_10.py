...
Autor: Claude
Version: 2.6 - Mit Black-Scholes Bewertung
"""
...
def black_scholes(S: float, K: float, T: float, r: float, sigma: float, 
    ...
    Args:
        ...
        r: Risikofreier Zinssatz (z.B. 0.05 für 5%)
        sigma: Volatilität (z.B. 0.20 für 20%)
    ...
    if sigma <= 0:
        sigma = 0.001  # Minimale Volatilität
...
def calculate_historical_volatility(prices: pd.Series, window: int = 30) -> float:
    ...
    Args:
        prices: Serie von Schlusskursen
        window: Anzahl Tage für Berechnung
    ...
    return max(0.05, min(1.0, annual_vol))  # Zwischen 5% und 100%
...
def run_simple_backtest(analyzer: StockAnalyzer, combination: dict,
                        ...
                        target_currency: str = 'CHF') -> dict:
    """
    Backtest mit Trade-Logging und Black-Scholes Schlussbewertung.
    
    - Optionen werden zum Mid-Preis gekauft/verkauft
    - Alle Trades werden geloggt
    - Am Ende: B&S Bewertung mit 30-Tage Volatilität
    """
    ...
    try:
        ...
        # Skalierung: historischer Startkurs -> aktueller Kurs
        ...
        call_mid = long_call['premium']
        put_mid = short_put['premium']
        hedge_mid = hedge_put['premium']
        
        call_strike = long_call['strike'] * scale
        put_strike = short_put['strike'] * scale
        hedge_strike = hedge_put['strike'] * scale
        
        call_days_orig = long_call.get('days', 180)
        put_days_orig = short_put.get('days', 365)
        hedge_days_orig = hedge_put.get('days', 90)
        
        call_delta = long_call.get('delta', 0.55)
        
        # Trade-Liste
        trades = []
        
        # === ERÖFFNUNGS-TRADES (Tag 1) ===
        ...
        short_call_qty = 0
        short_call_week_start = None
        
        # Preise für Volatilitätsberechnung sammeln
        price_history = []
        ...
        for i, (dt, row) in enumerate(bt_data.iterrows()):
            ...
            # Call-Abrechnung bei Wochenwechsel
            weekly_call_pl = 0
            if new_week and i > 0 and short_call_qty > 0:
                ...
                # Trade loggen: Schließen des alten Calls
                close_premium = max(0, price - short_call_strike) if price > short_call_strike else 0
                trades.append({
                    'date': dt,
                    'day': i + 1,
                    'action': 'RÜCKKAUF',
                    'type': 'Short Call (Schließen)',
                    'strike': short_call_strike,
                    'premium': close_premium,
                    'quantity': short_call_qty,
                    'total': -close_premium * 100 * short_call_qty,
                    'price': price,
                    'pl': weekly_call_pl
                })
            ...
            # Neuer Call bei Wochenbeginn
            if new_week:
                target = max(1, n // 2)
                if score >= 0.2:
                    short_call_qty = target
                    short_call_strike = price * 1.015  # 1.5% OTM
                    short_call_premium = price * 0.003  # ~0.3% Prämie
                    short_call_week_start = dt
                    
                    # Trade loggen: Neuen Call verkaufen
                    trades.append({
                        'date': dt,
                        'day': i + 1,
                        'action': 'VERKAUF',
                        'type': 'Short Call (Wöchentlich)',
                        'strike': short_call_strike,
                        'premium': short_call_premium,
                        'quantity': short_call_qty,
                        'total': short_call_premium * 100 * short_call_qty,
                        'price': price
                    })
                else:
                    short_call_qty = 0
            
            # Positions-Bewertung (vereinfacht)
            if price > call_strike:
                call_val = call_cost + (price - call_strike) * 100 * n
            else:
                call_val = max(0, call_cost + call_delta * (price - current_price) * 100 * n)
            
            if price < put_strike:
                put_loss = (put_strike - price) * 100 * n
            else:
                put_loss = 0
            
            if price < hedge_strike:
                hedge_val = (hedge_strike - price) * 100 * n
            else:
                hedge_val = 0
            
            base_val = call_val - put_loss + hedge_val + put_income - call_cost - hedge_cost
        ...
        # === SCHLUSSBEWERTUNG MIT BLACK-SCHOLES ===
        
        final_price = data[-1]['price']
        final_date = data[-1]['date']
        num_trading_days = len(data)
        
        # 30-Tage Volatilität berechnen
        vol_30d = calculate_historical_volatility(pd.Series(price_history), 30)
        
        # Risikofreier Zins (angenommen 4%)
        risk_free_rate = 0.04
        ...
        call_T = call_days_left / 365
        put_T = put_days_left / 365
        hedge_T = hedge_days_left / 365
        
        # Black-Scholes Bewertung
        long_call_bs = black_scholes(final_price, call_strike, call_T, risk_free_rate, vol_30d, 'call')
        short_put_bs = black_scholes(final_price, put_strike, put_T, risk_free_rate, vol_30d, 'put')
        hedge_put_bs = black_scholes(final_price, hedge_strike, hedge_T, risk_free_rate, vol_30d, 'put')
        ...
        trades.append({
            'date': final_date,
            'day': num_trading_days,
            'action': 'VERKAUF',
            'type': 'Long Call (B&S Schließen)',
            'strike': call_strike,
            'premium': long_call_bs,
            'quantity': n,
            'total': long_call_bs * 100 * n,
            'price': final_price,
            'method': 'Black-Scholes',
            'volatility': vol_30d,
            'days_left': call_days_left
        })
        
        # Short Put zurückkaufen
        trades.append({
            'date': final_date,
            'day': num_trading_days,
            'action': 'RÜCKKAUF',
            'type': 'Short Put (B&S Schließen)',
            'strike': put_strike,
            'premium': short_put_bs,
            'quantity': n,
            'total': -short_put_bs * 100 * n,
            'price': final_price,
            'method': 'Black-Scholes',
            'volatility': vol_30d,
            'days_left': put_days_left
        })
        ...
        trades.append({
            ...
            'day': num_trading_days,
            'action': 'VERKAUF',
            'type': 'Hedge Put (B&S Schließen)',
            'strike': hedge_strike,
            'premium': hedge_put_bs,
            'quantity': n,
            'total': hedge_put_bs * 100 * n,
            'price': final_price,
            'method': 'Black-Scholes',
            'volatility': vol_30d,
            'days_left': hedge_days_left
        })
        
        # Letzten Short Call abrechnen
        if short_call_qty > 0:
            ...
            trades.append({
                ...
                'type': 'Short Call (Wöchentlich)',
                'strike': short_call_strike,
                'premium': max(0, final_price - short_call_strike),
                'quantity': short_call_qty,
                'total': -max(0, final_price - short_call_strike) * 100 * short_call_qty,
                'price': final_price,
                'pl': final_call_pl
            })
        ...
        final_valuation = {
            ...
            'long_call': {
                'strike': call_strike,
                'days_left': call_days_left,
                'bs_value': long_call_bs,
                'total_value': long_call_bs * 100 * n
            },
            'short_put': {
                'strike': put_strike,
                'days_left': put_days_left,
                'bs_value': short_put_bs,
                'total_liability': short_put_bs * 100 * n
            },
            'hedge_put': {
                'strike': hedge_strike,
                'days_left': hedge_days_left,
                'bs_value': hedge_put_bs,
                'total_value': hedge_put_bs * 100 * n
            },
            'weekly_calls_income': total_call_income
        }
        
        # Gesamtergebnis nach B&S
        bs_total = (
            long_call_bs * 100 * n  # Long Call Wert
            - short_put_bs * 100 * n  # Short Put Kosten (Rückkauf)
            + hedge_put_bs * 100 * n  # Hedge Put Wert
            + total_call_income  # Alle wöchentlichen Call-Einnahmen
            - net_investment  # Ursprüngliche Investition
        )
        
        final_valuation['net_result'] = bs_total
        final_valuation['net_investment'] = net_investment
        
        # Update letzter Datenpunkt
        ...
        result['summary'] = {
            ...
            'price_change_pct': data[-1]['pct_change'],
            'investment': net_investment,
            'final_value': bs_total + net_investment,
            'net_result': bs_total,
            'strategy_pct': data[-1]['strategy_pct'],
            'stock_pct': data[-1]['stock_pct'],
            'call_income': total_call_income,
            'outperformance': data[-1]['strategy_pct'] - data[-1]['stock_pct'],
            'fx': fx,
...
def display_backtest(result: dict, curr_symbol: str, target_currency: str):
    ...
    c2.metric(f"Ende ({target_currency})", f"{curr_symbol}{s['end_price']*fx:.2f}", 
              f"{s['price_change_pct']:+.1f}%")
    c3.metric("Strategie", f"{s['strategy_pct']:+.1f}%", 
              f"{s['outperformance']:+.1f}% vs Aktie")
    ...
    if trades:
        ...
        for t in trades:
            trade_table.append({
                ...
                'Anzahl': t['quantity'],
                f'Gesamt ({target_currency})': f"{curr_symbol}{t['total']*fx:+,.2f}",
    ...
    # === BLACK-SCHOLES SCHLUSSBEWERTUNG ===
    st.markdown("### 📐 Schlussbewertung (Black-Scholes)")
    
    if final_val:
        st.info(f"**Volatilität (30 Tage):** {final_val.get('volatility_30d', 0)*100:.1f}% | **Risikofreier Zins:** {final_val.get('risk_free_rate', 0)*100:.1f}%")
        ...
        with val_cols[0]:
            ...
            st.write(f"B&S Wert: {curr_symbol}{lc.get('bs_value', 0)*fx:.2f}")
            st.success(f"**Gesamt: {curr_symbol}{lc.get('total_value', 0)*fx:,.2f}**")
        ...
        with val_cols[1]:
            ...
            st.write(f"B&S Wert: {curr_symbol}{sp.get('bs_value', 0)*fx:.2f}")
            st.error(f"**Rückkauf: {curr_symbol}{sp.get('total_liability', 0)*fx:,.2f}**")
        ...
        with val_cols[2]:
            ...
            st.write(f"B&S Wert: {curr_symbol}{hp.get('bs_value', 0)*fx:.2f}")
            st.success(f"**Gesamt: {curr_symbol}{hp.get('total_value', 0)*fx:,.2f}**")
        ...
        with sum_cols[0]:
            ...
            st.write(f"Long Call Verkauf: {curr_symbol}{final_val.get('long_call', {}).get('total_value', 0)*fx:,.2f}")
            st.write(f"Hedge Put Verkauf: {curr_symbol}{final_val.get('hedge_put', {}).get('total_value', 0)*fx:,.2f}")
            st.write(f"Wöchentliche Calls: {curr_symbol}{final_val.get('weekly_calls_income', 0)*fx:,.2f}")
        
        with sum_cols[1]:
            st.markdown("**📉 Ausgaben**")
            st.write(f"Short Put Rückkauf: {curr_symbol}{final_val.get('short_put', {}).get('total_liability', 0)*fx:,.2f}")
            st.write(f"Urspr. Investment: {curr_symbol}{final_val.get('net_investment', 0)*fx:,.2f}")
    ...
    if data:
        ...
        fig = make_subplots(
            ...
            vertical_spacing=0.1,
            row_heights=[0.6, 0.4],
            subplot_titles=(f'Kurs ({target_currency})', 'Rendite (%)')
        )
        
        # Kursverlauf
        fig.add_trace(
            go.Scatter(x=df['date'], y=df['price']*fx, name='Kurs',
                      line=dict(color='#2196f3', width=2)),
            row=1, col=1
        )
        
        # Strike-Linien
        fig.add_hline(y=s['call_strike']*fx, line_dash="dot", line_color="green",
                     annotation_text="Call Strike", row=1, col=1)
        fig.add_hline(y=s['put_strike']*fx, line_dash="dot", line_color="red",
                     annotation_text="Put Strike", row=1, col=1)
        fig.add_hline(y=s['hedge_strike']*fx, line_dash="dot", line_color="orange",
                     annotation_text="Hedge Strike", row=1, col=1)
        ...
        if buy_trades:
            fig.add_trace(
                go.Scatter(
                    ...
                    y=[t['price']*fx for t in buy_trades],
                    mode='markers',
                    name='Käufe',
        ...
        if sell_trades:
            fig.add_trace(
                go.Scatter(
                    ...
                    y=[t['price']*fx for t in sell_trades],
                    mode='markers',
                    name='Verkäufe',
        ...
        # Rendite
        fig.add_trace(
            go.Scatter(x=df['date'], y=df['strategy_pct'], name='Strategie',
                      line=dict(color='#4caf50', width=2)),
            row=2, col=1
        )
        fig.add_trace(
            go.Scatter(x=df['date'], y=df['stock_pct'], name='Aktie',
...
def main():
    ...
    st.markdown("*Analyse-Tool für sichere Rendite zur Rentenergänzung - Version 2.6 (Black-Scholes Backtest)*")
