...
def display_strategy_builder(analyzer: StockAnalyzer, metrics: dict,
    ...
    if len(result['combinations']) > 1:
        with st.expander("📊 Alternative Kombinationen"):
            for i, combo in enumerate(result['combinations'][1:5], 2):
                st.markdown(f"**Kombination {i}**")
                col1, col2, col3, col4 = st.columns(4)
                
                with col1:
                    st.write(f"Kontrakte: {combo['num_contracts']}")
                with col2:
                    st.write(f"Risiko: {curr_symbol}{combo['max_risk_chf']:,.0f}")
                with col3:
                    st.write(f"Rendite: {combo['premium_yield']:.1f}%")
                with col4:
                    st.write(f"Call-Strike: {curr_symbol}{combo['long_call']['strike'] * fx:.2f}")
...
def main():
    ...
    st.markdown("*Analyse-Tool für sichere Rendite zur Rentenergänzung - Version 2.3 (Strategie-Builder)*")
