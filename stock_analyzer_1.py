...
Autor: Claude
Version: 2.0 - Korrigierte Version
"""
...
def display_dividend_history(analyzer: StockAnalyzer):
    """Zeigt die Dividenden-Historie der letzten 10 Jahre"""
    ...
    display_df.columns = ['Jahr', 'Gesamt-Dividende ($)', 'Zahlungen', 
                          'Jahresanfangskurs ($)', 'Rendite (%)']
    
    # Formatierung
    display_df['Gesamt-Dividende ($)'] = display_df['Gesamt-Dividende ($)'].apply(lambda x: f"${x:.4f}")
    display_df['Jahresanfangskurs ($)'] = display_df['Jahresanfangskurs ($)'].apply(
    ...
    with st.expander("📋 Einzelne Dividendenzahlungen pro Jahr"):
        for _, row in div_history.iterrows():
            ...
            if payments:
                payments_str = ", ".join([f"${p:.4f}" for p in payments])
...
def display_options_analysis(analyzer: StockAnalyzer, metrics: dict):
    ...
    if strategy['options'] is not None and not strategy['options'].empty:
        ...
        if not relevant_calls.empty:
            ...
            display_df.columns = ['Strike', 'Letzter Preis', 'Bid', 'Ask', 'Open Interest', 'IV (%)']
            st.dataframe(display_df.round(2), use_container_width=True, hide_index=True)
            
            # Empfehlung
            best_call = relevant_calls.iloc[0]
            st.success(f"💡 Empfehlung: Strike ${best_call['strike']:.2f} (ATM/leicht ITM) - "
                      f"Prämie ca. ${best_call['ask']:.2f} - Hohe Liquidität")
    else:
    ...
    if strategy['options'] is not None and not strategy['options'].empty:
        ...
        if not relevant_puts.empty:
            ...
            st.success(f"💡 Empfehlung: Strike ${best_put['strike']:.2f} ({((best_put['strike']/current_price)-1)*100:.1f}% OTM) - "
                      f"Prämie ca. ${best_put['bid']:.2f} - Hohe Prämie!")
    else:
    ...
    if strategy['options'] is not None and not strategy['options'].empty:
        ...
        if not relevant_puts.empty:
            ...
            st.info(f"💡 Empfehlung: Strike ${best_hedge['strike']:.2f} ({((best_hedge['strike']/current_price)-1)*100:.1f}% OTM) - "
                   f"Kosten ca. ${best_hedge['ask']:.2f} - Günstige Absicherung")
    else:
    ...
    if strategy['options'] is not None and not strategy['options'].empty:
        ...
        if not relevant_calls.empty:
            ...
            # Empfehlung
            best_call = relevant_calls.iloc[0]
            st.success(f"💡 Empfehlung: Strike ${best_call['strike']:.2f} ({((best_call['strike']/current_price)-1)*100:.1f}% OTM) - "
                      f"Prämie ca. ${best_call['bid']:.2f}")
    else:
    ...
    if comparison_data:
        ...
        total_credit = sum([float(d['Prämie'].replace('$', '')) for d in comparison_data if d['Cash Flow'] == 'CREDIT'])
        total_debit = sum([float(d['Prämie'].replace('$', '')) for d in comparison_data if d['Cash Flow'] == 'DEBIT'])
        net_cash = total_credit - total_debit
        
        if net_cash > 0:
            st.success(f"💰 Netto Credit (Einnahme): ${net_cash:.2f} pro Aktie / ${net_cash*100:.2f} pro Kontrakt")
        else:
            st.warning(f"💸 Netto Debit (Ausgabe): ${abs(net_cash):.2f} pro Aktie / ${abs(net_cash)*100:.2f} pro Kontrakt")
...
def main():
    ...
    st.markdown("*Analyse-Tool für sichere Rendite zur Rentenergänzung - Version 2.0*")
            ...
            with term_cols[0]:
                ...
                if dates_info['ex_dividend_estimated']:
                    ...
                    if dates_info['estimated_dividend']:
                        st.write(f"   Geschätzte Dividende: ${dates_info['estimated_dividend']:.4f}")
                else:
