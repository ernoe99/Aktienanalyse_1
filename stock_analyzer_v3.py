...
def display_strategy_builder_with_comparison(analyzer: StockAnalyzer, metrics: dict,
    ...
    # Mindest-Prämie für Call-Verkäufe
    st.divider()
    ...
    if st.button("🔄 Strategien berechnen", type="primary", use_container_width=True):
        ...
        if result_a['success'] and result_b['success']:
            ...
            # Risiko-Vergleich
            risk_a = combo_a['max_risk_chf']
            risk_b = combo_b['max_risk_chf']
            
            if abs(risk_a - risk_b) < 500:
                st.info("Das Risikoprofil beider Strategien ist ähnlich.")
            elif risk_a < risk_b:
                st.info(f"Strategie A hat ein niedrigeres Risiko ({curr_symbol}{risk_a:,.0f} vs. {curr_symbol}{risk_b:,.0f}).")
            else:
                st.info(f"Strategie B hat ein niedrigeres Risiko ({curr_symbol}{risk_b:,.0f} vs. {curr_symbol}{risk_a:,.0f}).")
