...
Autor: Claude
Version: 2.9 - OTM + ATM/ITM Strategien mit Call-Verkaufsregeln
"""
    ...
    for _, call in atm_calls.iterrows():
        for _, put in otm_puts.iterrows():
            for _, hedge in hedge_options.iterrows():
                ...
                # Wenn Risiko negativ oder 0, setze auf minimales Risiko
                if max_risk_per_contract <= 0:
                    max_risk_per_contract = abs(net_cost) if net_cost < 0 else call_cost
                
                # Sicherheit: Falls immer noch 0, überspringe diese Kombination
                if max_risk_per_contract <= 0:
                    max_risk_per_contract = max(1, put['strike'] * 10)  # Fallback: 10% des Put-Strikes
...
def calculate_strategy_combinations_atm_put(analyzer: StockAnalyzer, metrics: dict, 
                                            target_risk: float = 5000,
                                            source_currency: str = 'USD',
                                            target_currency: str = 'CHF') -> dict:
    """
    Berechnet Optionskombinationen mit ATM/ITM Short Puts (Cash-Secured Put Strategie):
    
    1. Long Call (6-12 Mo) - ATM bis leicht ITM
    2. Short Put (12-24 Mo) - ATM/ITM (95-102% des Kurses) - HÖHERE PRÄMIE
    3. Long Hedge Put (3-6 Mo) - OTM für Absicherung
    
    Vorteile ATM/ITM Puts:
    - Höhere Prämieneinnahme
    - Bessere Chance auf Aktienübernahme bei gewünschtem Einstieg
    - Mehr Zeitwert zum Verfall
    
    Nachteile:
    - Höheres Risiko bei starkem Kursrückgang
    - Weniger Puffer nach unten
    """
    ...
    # *** GEÄNDERT: Short Puts ATM/ITM (95-102% des aktuellen Kurses) ***
    short_puts = short_put['options']
    atm_itm_puts = short_puts[(short_puts['strike'] >= current_price * 0.95) & 
                              (short_puts['strike'] <= current_price * 1.02)]
    atm_itm_puts = atm_itm_puts.sort_values('bid', ascending=False).head(3)
    
    # Hedge Puts filtern (15-25% OTM für besseren Schutz bei ATM-Strategie)
    hedge_puts = hedge_put['options']
    hedge_options = hedge_puts[(hedge_puts['strike'] >= current_price * 0.75) & 
                               (hedge_puts['strike'] <= current_price * 0.88)]
    hedge_options = hedge_options.sort_values('ask', ascending=True).head(3)
    ...
    # Alle Kombinationen durchgehen
    for _, call in atm_calls.iterrows():
        for _, put in atm_itm_puts.iterrows():
            for _, hedge in hedge_options.iterrows():
                # Kosten und Einnahmen berechnen
                ...
                # Berechne ITM/OTM Status
                put_moneyness = (put['strike'] / current_price - 1) * 100  # positiv = ITM
                ...
                combination = {
                    ...
                    'short_put': {
                        ...
                        'premium': put['bid'],
                        'iv': put.get('impliedVolatility', 0) * 100,
                        'moneyness_pct': put_moneyness,  # positiv=ITM, negativ=OTM
                        'moneyness_type': 'ITM' if put_moneyness > 0.5 else ('ATM' if abs(put_moneyness) <= 0.5 else 'OTM')
                    },
                    'hedge_put': {
                        'strike': hedge['strike'],
                        'expiry': hedge_put.get('expiration', 'N/A'),
    ...
    # Nach Prämienrendite sortieren
    combinations.sort(key=lambda x: x['premium_yield'], reverse=True)
    result['combinations'] = combinations[:5]
...
def display_strategy_builder(analyzer: StockAnalyzer, metrics: dict,
    ...
    # =====================================================
    # CALL-VERKAUFSREGELN (Dokumentation)
    # =====================================================
    with st.expander("📚 **Regeln für wöchentliche Call-Verkäufe**", expanded=False):
        st.markdown("""
        ### Systematische Call-Verkäufe zur Prämienmaximierung
        
        **Grundregel:** Verkaufe wöchentlich 50% der Long-Call-Position als Short Calls (Covered Calls)
        
        ---
        
        #### 📊 Signale für Call-Verkauf
        
        | Signal | Bedingung | Empfehlung |
        |--------|-----------|------------|
        | **MACD** | MACD unter Signallinie | ✅ Calls verkaufen (bearish) |
        | **MACD** | MACD über Signallinie | ⚠️ Vorsicht (bullish) |
        | **200-SMA** | Kurs > 5% über SMA200 | ✅ Calls verkaufen (überkauft) |
        | **200-SMA** | Kurs < 5% unter SMA200 | ❌ Keine Calls (überverkauft) |
        | **Saisonalität** | Historisch schwache Woche | ✅ Calls verkaufen |
        | **Saisonalität** | Historisch starke Woche | ⚠️ Weniger Calls |
        
        ---
        
        #### 🎯 Strike-Auswahl
        
        | Marktlage | Strike-Empfehlung | Begründung |
        |-----------|-------------------|------------|
        | **Neutral/Bearish** | 1-2% OTM | Maximale Prämie bei Schutz |
        | **Bullish** | 2-3% OTM | Mehr Aufwärtspotenzial |
        | **Hohe Volatilität** | 1.5-2.5% OTM | Höhere Prämien nutzen |
        | **Niedrige Volatilität** | 0.5-1.5% OTM | Näher am Geld für Prämie |
        
        ---
        
        #### ⏰ Timing
        
        - **Eröffnung:** Montag/Dienstag nach Marktöffnung
        - **Laufzeit:** 5-7 Tage (wöchentlich)
        - **Schließung:** Automatisch bei Verfall ODER
        - **Vorzeitige Schließung:** Bei 50% Gewinn oder drohendem ITM-Verfall
        
        ---
        
        #### 📈 Prämienberechnung (Black-Scholes)
        
        Die Prämie wird nach Black-Scholes mit der aktuellen 30-Tage-Volatilität berechnet:
        - **Typische Prämie:** 0.2-0.5% des Aktienwerts pro Woche
        - **Annualisiert:** 10-25% zusätzliche Rendite
        
        ---
        
        #### ⚠️ Risikomanagement
        
        1. **Nie mehr als 50%** der Position abdecken
        2. **Rollen statt Assignment:** Bei ITM vor Verfall rollen
        3. **Earnings vermeiden:** Keine Calls über Quartalszahlen
        4. **Dividende beachten:** Achtung bei Ex-Dividend-Datum
        """)
    ...
    # =====================================================
    # ZWEI STRATEGIE-VARIANTEN ALS TABS
    # =====================================================
    strategy_tab1, strategy_tab2 = st.tabs([
        "📉 **Konservativ** (OTM Put)", 
        "💰 **Aggressiv** (ATM/ITM Put)"
    ])
    
    # Kombinationen für beide Strategien berechnen
    with st.spinner("Berechne optimale Kombinationen..."):
        result_otm = calculate_strategy_combinations(
            analyzer, metrics, target_risk, source_currency, target_currency
        )
        result_atm = calculate_strategy_combinations_atm_put(
            analyzer, metrics, target_risk, source_currency, target_currency
        )
    
    # =====================================================
    # TAB 1: KONSERVATIVE STRATEGIE (OTM Put)
    # =====================================================
    with strategy_tab1:
        st.markdown("""
        <div style="background-color: #e8f5e9; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
        <b>🛡️ Konservative Strategie:</b> Short Put 10-20% OTM<br>
        <small>Größerer Sicherheitspuffer, niedrigere Prämie, weniger Risiko bei Kursrückgang</small>
        </div>
        """, unsafe_allow_html=True)
        
        if not result_otm['combinations']:
            st.warning("Keine geeigneten OTM-Kombinationen gefunden.")
        else:
            display_strategy_details(result_otm, metrics, target_currency, curr_symbol, "OTM", analyzer, source_currency)
    
    # =====================================================
    # TAB 2: AGGRESSIVE STRATEGIE (ATM/ITM Put)
    # =====================================================
    with strategy_tab2:
        st.markdown("""
        <div style="background-color: #fff3e0; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
        <b>💰 Aggressive Strategie:</b> Short Put ATM/ITM (95-102% vom Kurs)<br>
        <small>Höhere Prämie, bessere Chance auf Aktienübernahme, mehr Risiko bei Kursrückgang</small>
        </div>
        """, unsafe_allow_html=True)
        
        if not result_atm['combinations']:
            st.warning("Keine geeigneten ATM/ITM-Kombinationen gefunden.")
        else:
            display_strategy_details(result_atm, metrics, target_currency, curr_symbol, "ATM", analyzer, source_currency)
    
    # =====================================================
    # VERGLEICH BEIDER STRATEGIEN
    # =====================================================
    if result_otm['combinations'] and result_atm['combinations']:
        st.divider()
        st.subheader("📊 Strategievergleich")
        
        best_otm = result_otm['best_combination']
        best_atm = result_atm['best_combination']
        fx = result_otm['fx_rate']
        
        comp_cols = st.columns(2)
        
        with comp_cols[0]:
            st.markdown("**📉 Konservativ (OTM)**")
            st.write(f"Short Put Strike: {curr_symbol}{best_otm['short_put']['strike']*fx:.2f} ({best_otm['short_put']['otm_pct']:.1f}% OTM)")
            st.write(f"Prämienrendite: **{best_otm['premium_yield']:.1f}%** p.a.")
            st.write(f"Max. Risiko: {curr_symbol}{best_otm['max_risk_chf']:,.0f}")
        
        with comp_cols[1]:
            st.markdown("**💰 Aggressiv (ATM/ITM)**")
            moneyness = best_atm['short_put'].get('moneyness_type', 'ATM')
            moneyness_pct = best_atm['short_put'].get('moneyness_pct', 0)
            st.write(f"Short Put Strike: {curr_symbol}{best_atm['short_put']['strike']*fx:.2f} ({moneyness}, {moneyness_pct:+.1f}%)")
            st.write(f"Prämienrendite: **{best_atm['premium_yield']:.1f}%** p.a.")
            st.write(f"Max. Risiko: {curr_symbol}{best_atm['max_risk_chf']:,.0f}")
        
        # Prämien-Differenz
        premium_diff = best_atm['premium_yield'] - best_otm['premium_yield']
        risk_diff = best_atm['max_risk_chf'] - best_otm['max_risk_chf']
        
        if premium_diff > 0:
            st.info(f"💡 Die aggressive Strategie bringt **{premium_diff:.1f}% mehr Prämienrendite**, "
                   f"aber **{curr_symbol}{risk_diff:+,.0f}** {'mehr' if risk_diff > 0 else 'weniger'} Risiko.")


def display_strategy_details(result: dict, metrics: dict, target_currency: str, 
                            curr_symbol: str, strategy_type: str, analyzer: StockAnalyzer,
                            source_currency: str):
    """Zeigt Details einer Strategie-Kombination an."""
    ...
    with comp_cols[1]:
        ...
        sp = best['short_put']
        if strategy_type == "ATM":
            moneyness = sp.get('moneyness_type', 'ATM')
            moneyness_pct = sp.get('moneyness_pct', 0)
            st.write(f"Strike: {curr_symbol}{sp['strike'] * fx:.2f} ({moneyness}, {moneyness_pct:+.1f}%)")
        else:
    ...
    with bt_col3:
        st.metric(
            "Aktueller Kurs",
            f"{curr_symbol}{current_price * fx:.2f}",
...
def main():
    ...
    st.markdown("*Analyse-Tool für sichere Rendite zur Rentenergänzung - Version 2.9 (OTM + ATM/ITM Strategien)*")
