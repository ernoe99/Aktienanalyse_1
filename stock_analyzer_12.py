...
Autor: Claude
Version: 2.7 - Echte Strikes aus Optionskette & Black-Scholes Bewertung
"""
...
def get_available_strikes(analyzer: StockAnalyzer, scale: float = 1.0) -> list:
    """Holt verfügbare Strikes aus der Optionskette."""
    strikes = []
    try:
        options = analyzer.get_options_chain()
        if options and 'calls' in options and not options['calls'].empty:
            all_strikes = sorted(options['calls']['strike'].unique())
            strikes = [s * scale for s in all_strikes]
    except:
        pass
    return strikes
...
def find_nearest_strike(price: float, available_strikes: list, direction: str = 'above') -> float:
    """Findet den nächsten verfügbaren Strike."""
    if not available_strikes:
        if price < 50:
            base = round(price * 2) / 2
        else:
            base = round(price)
        return base * 1.01 if direction == 'above' else base * 0.99
...
def run_simple_backtest(analyzer: StockAnalyzer, combination: dict,
                        ...
                        source_currency: str = 'USD',
                        target_currency: str = 'CHF') -> dict:
    """Backtest mit echten Strikes und Black-Scholes Bewertung für jeden Trade."""
    
    result = {'success': False, 'data': [], 'trades': [], 'final_valuation': {}, 'summary': {}, 'error': None}
    ...
    try:
        ...
        available_strikes = get_available_strikes(analyzer, scale)
...
def main():
    ...
    st.markdown("*Analyse-Tool für sichere Rendite zur Rentenergänzung - Version 2.7 (Echte Strikes & B&S)*")
